...
import { MARKDOWN_ARTICLE_TEMPLATE, normalizeMarkdown } from '@/utils/normalizeMarkdown'
...
function insertTemplate() {
  ...
  form.value.content = MARKDOWN_ARTICLE_TEMPLATE
}
...
async function onSubmit() {
  ...
  try {
    const res = isEdit.value
      ? await updateBlog(String(route.params.slug), form.value)
...
<template>
  <div class="admin-page">
    <header class="admin-page__header">
      ...
      <button type="button" @click="router.push('/admin/blog')">返回列表</button>
    </header>
    ...
    <form class="admin-form" novalidate @submit.prevent="onSubmit">
      ...
      <div class="admin-form__editor">
        ...
        <p class="admin-form__editor-hint">
          建议使用 <code>##</code> 二级标题划分章节，<code>###</code> 三级标题划分子节，段与段之间空一行。
        </p>
