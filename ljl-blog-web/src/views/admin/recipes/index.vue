...
import { deleteRecipe, fetchRecipeList, importRecipeSeed } from '@/api/modules/recipe'
import type { Article } from '@/types'

const router = useRouter()
const loading = ref(false)
const seeding = ref(false)
const recipes = ref<Article[]>([])
const keyword = ref('')
const page = ref(1)
const pageSize = 10
const total = ref(0)
...
async function onDelete(slug: string) {
  ...
  try {
    ...
    if (res.code === 0) {
      ...
      if (recipes.value.length === 1 && page.value > 1) {
        page.value -= 1
      }
      await loadList()
    } else {
...
async function onImportSeed() {
  ...
  const message = replace
    ? '将清空现有菜谱并导入 3 条示例数据，是否继续？'
    : '将导入 3 条示例菜谱数据，是否继续？'
  if (!window.confirm(message)) return

  seeding.value = true
  try {
    const res = await importRecipeSeed(replace)
    if (res.code === 0) {
  ...
  <div class="admin-page">
    <header class="admin-page__header">
      ...
      <div class="admin-page__actions">
        ...
        <button type="button" class="admin-page__primary" @click="router.push('/admin/recipes/new')">
          新建菜谱
        </button>
      </div>
    </header>
    ...
    <form class="admin-search" @submit.prevent="onSearch">
      <input v-model="keyword" type="search" placeholder="搜索菜名、摘要、标签..." />
    ...
    <template v-else>
      <table class="admin-table">
        ...
        <tbody>
          <tr v-if="recipes.length === 0">
            <td colspan="5" class="admin-table__empty">暂无菜谱</td>
          </tr>
          <tr v-for="item in recipes" :key="item.id">
            <td>{{ item.title }}</td>
            <td>{{ item.category }}</td>
            <td>{{ item.readTime ? `${item.readTime} 分钟` : '—' }}</td>
            <td>{{ item.publishDate }}</td>
            <td class="admin-table__actions">
              <button type="button" @click="router.push(`/admin/recipes/${item.slug}/edit`)">编辑</button>
      ...
      <div v-if="totalPages > 1" class="admin-pagination">
        ...
        <span>{{ page }} / {{ totalPages }}（共 {{ total }} 道）</span>
