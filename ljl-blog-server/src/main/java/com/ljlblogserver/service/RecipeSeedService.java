...
public class RecipeSeedService {
    ...
    public int importSeed(boolean replace) throws Exception {
        if (replace) {
            articleService.deleteAll(ArticleType.RECIPE);
        } else if (articleMapper.count(ArticleType.RECIPE.getValue(), null, null, null) > 0) {
            return 0;
        }

        ClassPathResource listResource = new ClassPathResource("seed/recipes.json");
        ClassPathResource contentResource = new ClassPathResource("seed/recipes-content.json");
        ...
        for (JsonNode node : list) {
            ...
            request.setReadTime(node.path("readTime").asInt(30));
            request.setFeatured(node.path("featured").asBoolean(false));
            request.setTags(readTags(node.get("tags")));

            articleService.create(ArticleType.RECIPE, request);
            imported++;
        }
        return imported;
    }
